function render(event, data) {
	...
	//3
		...
		$("#celebContainer").css("display", "none");
	}
	//4
	...
	//5
		...
		$("#message").html(
			"<h3>You're up " + gameState.currentDescriber + "!</h3>"
		);
	}
	//6
		...
		if (gameState.myTurn === true) {
			...
			gameState.team1.includes(gameState.currentDescriber) &&
			gameState.team1.includes(gameState.playerName)
		) {
			$("#message").html("<h2>Guess for Team 1!</h2>");
		} else if (
			gameState.team2.includes(gameState.currentDescriber) &&
			gameState.team2.includes(gameState.playerName)
		) {
	...
	//8
		...
		$("#nextCelebButton, #passCelebButton").show();
	...
	//14
		...
		console.log(
			"Unhandled Event " + event + " and Unhandled Type " + data.type
		);
	}
}
